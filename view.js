...
$('.btnStartServer').on('click', () => {
	...
	func.getResult(function(resultData){
		if(resultData.length < 1){
			...
			dialog.showMessageBox({
				type: "question",
				buttons:["Annuleren", "Verdergaan", "Reset"],
				message: "Huidige sessie heeft opgenomen cue's",
				detail: "Wil je de huidige sessie resetten of verdergaan met opnemen?"
			}, function(response){
				if(response == 1){
					startServer({reset: false})
				}else if(response == 2){
					startServer({reset: true})
				}
			});
...
$('.btnSave').on('click', function(){
	// Check of er wat is opgenomen
	func.getResult(function(resultData){
		if(resultData.length < 1){
			dialog.showMessageBox({
				type: "error",
				buttons:["Ok"],
				message: "Er is niks opgenomen.",
				detail: "Dus ook niks om op te slaan."
			});
		}else{
			dialog.showSaveDialog({
					title: "Ondertitel Opslaan",
					defaultPath: '~/Showstudy.ass',
				}, (fileName) => {
